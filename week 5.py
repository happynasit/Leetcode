...
class BinarySearchTree:
    ...
    def remove(self, item: Any) -> None:
        ...
        """
        if self.is_empty():
            # skips over to the other branch
            pass
        elif item == self._root:
            self._delete_root()
        elif item < self._root:
    ...
    def _extract_max(self) -> Any:
        """
        a helper to extract the maximum value from the left side
        """
        if self._right.is_empty:
            root = self._root
            self._root = None
            # the same for left and rigt
            return root
        else:
    ...
    def _delete_root(self) -> None:
        ...
        elif self._right.is_empty() and not self._left.is_empty():
